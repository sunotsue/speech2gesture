...
class Skeleton2D(Modality):
  def __init__(self, path2data='projects/dataset_processed/pats-plus-copied/pats-plus-clean/',
               #path2data='../dataset/groot/data/speech2gesture_data',
               path2outdata='projects/dataset_processed/pats-plus-copied/pats-plus-clean/',
               speaker='all',
...
#      pdb.set_trace()
  ...
  def joint_LR(self, LR, mask=[]):
    mask_neg = sorted(set(range(len(self.joint_names))) - set(mask))
    joints = []
    joints_neg = [LR + j for j in ['Eye', 'Shoulder']] ## do not translate the shoulders and eye
    for i, joint in enumerate(mask_neg):
      if self.joint_names[joint][0] == LR and self.joint_names[joint] not in joints_neg:
        joints.append(i)
    return joints

  def joint_right(self, mask=[]):
    return self.joint_LR(LR='R', mask=mask)

  def joint_left(self, mask=[]):
    return self.joint_LR(LR='L', mask=mask)
