...
class Data(Modality):
  ...
  def __init__(self, path2data, speaker,
    ...
    self.audio_average=audio_average
...
#      self.df = self.get_df_subset('speaker', speaker)
  ...
  def minidataKwargs(self):
    minidataKwargs = {'modalities':self.modalities,
                      ...
                      'audio_average':self.audio_average}      
...
class MiniData(Dataset, HDF5):
  def __init__(self, path2h5, modalities, fs_new, time, modality_classes, window_hop, style=0, repeat_text=1, text_in_modalities=False, filler=0, audio_average=0, **kwargs):
  ...
  def _window_average(self, data, start, end, interval):
    extra = int((end-start)/interval + 1) * interval - end + start
    e_ = extra//2
    s_ = extra - e_
    dd = data[start:end]
    return np.concatenate([dd[:s_], dd, dd[-e_:]], axis=0).reshape(-1, interval, dd.shape[-1]).mean(axis=1)
  ...
  def __getitem__(self, idx):
    ...
    for i, modality in enumerate(self.modalities):
      ...
      if self.audio_average and 'audio' in modality:
        item[modality] = self._window_average(data, start, end, interval).astype(np.float64)
      else:
