...
class ZNorm():
  ...
  Example::
    ...
    pre = ZNorm(['audio', 'pose'], savepath='./work/suminpar/speech2ges/speech2gesture/src/preprocessing_temp', data=data)
    for batch in data:
      break
  ...
  '''
  def __init__(self, variable_list=[], savepath='./work/suminpar/speech2ges/speech2gesture/src/preprocessing/muvar', key='key', data=None, num_dims=2, verbose=True, relative2parent=0, pre=None):
...
class PoseStarts():
  ...
  '''
  def __init__(self, variable_list=[], savepath='./work/suminpar/speech2ges/speech2gesture/src/preprocessing/poseStarts', key='key', data=None, num_dims=2, verbose=True, pre=None):
    os.makedirs(savepath, exist_ok=True)
    self.savepath = savepath
    self.key = '_'.join(key) if isinstance(key, list) else key
    self.data = data
...
class KMeans():
  ...
  Example::
    ...
    device = 'cuda'
    data = DataLoader(DummyData(variable_list=variable_list, random=True), batch_size=1000)
    kmeans = KMeans(savepath='./work/suminpar/speech2ges/speech2gesture/src/preprocessing_temp', data=data, device=device)
    for batch in data:
      break
    predictions = kmeans(batch['pose/data'])
  '''
  def __init__(self, variable_list = [], savepath='./work/suminpar/speech2ges/speech2gesture/src/preprocessing/kmeans', key='key', data=None, num_clusters=8, mask=[0, 7, 8, 9], feats=['pose', 'velocity'], verbose=True):
    os.makedirs(savepath, exist_ok=True)
    self.variable_list = variable_list
    ...
    # self.muvarpath = './work/suminpar/speech2ges/speech2gesture/src/preprocessing/muvar'
    # self.hdf5 = HDF5()
    # self.variable_dict = {}
...
class GMM(KMeans):
  def __init__(self, variable_list = [], savepath='./work/suminpar/speech2ges/speech2gesture/src/preprocessing/kmeans', key='key', data=None, num_clusters=8, mask=[0, 7, 8, 9], feats=['pose', 'velocity'], verbose=True):
    super().__init__(variable_list=variable_list, savepath='./work/suminpar/speech2ges/speech2gesture/src/preprocessing/gmm', key=key, data=data, num_clusters=num_clusters, mask=mask, feats=feats, verbose=verbose)
...
if __name__ == '__main__':
  ...
  pre = ZNorm(['pose'], savepath='./work/suminpar/speech2ges/speech2gesture/src/preprocessing_temp', data=data)
  for batch in data:
    break
  mean = pre(batch)['pose'].mean()
  std = pre(batch)['pose'].std()
