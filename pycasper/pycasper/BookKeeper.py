...
class BookKeeper():
  ...
  def __init__(self, args, args_subset,
    ...
    print('self load pretrained model',self.load_pretrained_model)
    ...
    if self.args.load:
      ...
      #self.save_dir = os.path.join('work/suminpar/speech2ges/speech2gesture/src/',Path(args.load).parent.as_posix())
      # 에러나면 이거 시도 하기: 
      self.save_dir = self.args.save_dir
      print('SELF SAVE DIR',self.save_dir)
      ...
      ## Serialize and save args
        print('dont load pretrained model')
        self._save_args()
  ...
  def _update_exp(self):
    if self.args.exp is not None:
      exp = 0
      exp_file = 'work/suminpar/speech2ges/speech2gesture/src/.experiments'
  ...
  def _load_name(self):
    ...
    print('LOAD NAME PRE JOIN',name_filepath)
    name_filepath = os.path.join('work/suminpar/speech2ges/speech2gesture/src/', name_filepath) 
    print('LOAD NAME POST JOIN',name_filepath)
    return pkl.load(open(name_filepath, 'rb'))

  def _save_name(self):
    name_filepath = self.name(self.name_ext[0], self.name_ext[1], self.save_dir)
    print('SAVE NAME',name_filepath)
    #name_filepath = os.path.join('work/suminpar/speech2ges/speech2gesture/src/', name_filepath) 
    pkl.dump(self.name, open(name_filepath, 'wb'))

  def _load_res(self):
    res_filepath = self.name(self.res_ext[0], self.res_ext[1], self.save_dir)
    print('LOAD RES',res_filepath)
    #res_filepath = os.path.join('work/suminpar/speech2ges/speech2gesture/src/', res_filepath) 
  ...
  def _load_model(self, model, map_location=torch.device('cpu')):
    ...
    print('WEIGTS PATH',weights_path)
